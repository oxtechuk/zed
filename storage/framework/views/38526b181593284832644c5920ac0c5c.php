...
<header class="crm-topbar">
    ...
    <div class="dropdown">
        <div class="crm-topbar-user" id="userDropdown" data-bs-toggle="dropdown" aria-expanded="false" style="cursor: pointer;">
            <div class="crm-user-avatar shadow-sm">
                <?php if($currentUser?->avatar): ?>
                    <img src="<?php echo e(asset('storage/'.$currentUser->avatar)); ?>" alt="<?php echo e($currentUser->name); ?>">
                <?php else: ?>
                    <?php echo e(strtoupper(substr($currentUser?->name ?? 'A', 0, 1))); ?>

                <?php endif; ?>
            </div>
            <div class="d-none d-md-block">
                <div class="crm-user-name"><?php echo e($currentUser?->name ?? 'Admin'); ?></div>
            </div>
        ...
        <ul class="dropdown-menu dropdown-menu-start shadow-lg border-0 rounded-4 mt-2 py-2" aria-labelledby="userDropdown" style="min-width: 200px;">
            <li class="px-3 py-2 border-bottom mb-2">
                <div class="fw-bold small text-muted mb-1"><?php echo e(__('مرحباً بك')); ?></div>
                <div class="fw-bold text-dark"><?php echo e($currentUser?->name); ?></div>
            </li>
            <li>
                <a class="dropdown-item d-flex align-items-center gap-2 py-2 px-3" href="<?php echo e(route('crm.profile.index')); ?>">
                    <i class="bi bi-person-circle text-primary fs-5"></i>
                    <span><?php echo e(__('الملف الشخصي')); ?></span>
                </a>
            </li>
            <?php if($currentUser?->isAdmin()): ?>
            <li>
                <a class="dropdown-item d-flex align-items-center gap-2 py-2 px-3" href="<?php echo e(route('crm.settings.general')); ?>">
                    <i class="bi bi-gear-wide-connected text-primary fs-5"></i>
                    <span><?php echo e(__('إعدادات النظام')); ?></span>
                </a>
            </li>
            <?php endif; ?>
    ...
    <div class="crm-topbar-search" style="position: relative;">
        <input type="text" id="global-search-input" placeholder="<?php echo e(__('بحث سريع (عملاء، طلبات، موظفين)...')); ?>" autocomplete="off">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateBadge() {
            fetch(`<?php echo e(route('crm.notifications.index')); ?>?only_count=1`)
                .then(r => r.json())
                .then(data => {
        ...
        function fetchNotifications() {
            ...
            notifContainer.innerHTML = `<div class="p-4 text-center text-muted"><div class="spinner-border spinner-border-sm mb-2"></div><div><?php echo e(__('جاري التحميل...')); ?></div></div>`;
            fetch(`<?php echo e(route('crm.notifications.index')); ?>`)
                .then(r => r.json())
                .then(data => {
                    if (data && data.notifications && data.notifications.length > 0) {
                        ...
                        notifContainer.innerHTML = `<div class="p-5 text-center text-muted small"><i class="bi bi-bell-slash fs-2 d-block mb-2 opacity-25"></i><?php echo e(__('لا توجد إشعارات جديدة')); ?></div>`;
                    }
                }).catch(e => {
                    console.error('Fetch notifications failed', e);
                    notifContainer.innerHTML = `<div class="p-4 text-center text-danger small"><?php echo e(__('حدث خطأ أثناء التحميل')); ?></div>`;
                });
        }
        ...
        window.markAsRead = function(id, event) {
            fetch(`<?php echo e(url('/crm/notifications')); ?>/${id}/read`, {
                method: 'POST',
        ...
        window.markAllAsRead = function() {
            fetch(`<?php echo e(route('crm.notifications.read-all')); ?>`, {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }
        ...
        if (searchInput && searchResults) {
            searchInput.addEventListener('input', function() {
                ...
                debounceTimer = setTimeout(() => {
                    fetch(`<?php echo e(route('crm.global-search')); ?>?query=${encodeURIComponent(query)}`)
                        ...
                        .then(data => {
                            ...
                            if (data && data.length > 0) {
                                ...
                                searchResults.innerHTML = `<div class="crm-search-no-results"><?php echo e(__('لا توجد نتائج مطابقة...')); ?></div>`;
                            }
    ...
    <div class="crm-topbar-end">
        <a href="<?php echo e(route('lang.switch', app()->getLocale() == 'ar' ? 'en' : 'ar')); ?>"
           ...
           rel="prefetch"
           title="<?php echo e(app()->getLocale() == 'ar' ? 'Switch to English' : 'التحويل للعربية'); ?>">
            <i class="bi bi-translate"></i>
            <span class="small fw-bold ms-1"><?php echo e(app()->getLocale() == 'ar' ? 'EN' : 'عربي'); ?></span>
        </a>
        <div class="dropdown">
            <button class="crm-topbar-btn" id="notif-dropdown-btn" data-bs-toggle="dropdown" aria-expanded="false" title="<?php echo e(__('الإشعارات')); ?>">
            ...
            <div class="dropdown-menu dropdown-menu-end crm-notif-dropdown p-0 shadow-lg border-0" aria-labelledby="notif-dropdown-btn">
                <div class="p-3 border-bottom d-flex justify-content-between align-items-center">
                    <h6 class="mb-0 fw-bold"><?php echo e(__('الإشعارات')); ?></h6>
                    <button class="btn btn-link btn-sm p-0 text-decoration-none small" onclick="markAllAsRead()"><?php echo e(__('تحديد الكل كمقروء')); ?></button>
                </div>
                <div id="notif-items-container" class="notif-scroll-area">
                    
                    <div class="p-4 text-center text-muted">
                        <div class="spinner-border spinner-border-sm mb-2" role="status"></div>
                        <div class="small"><?php echo e(__('جاري التحميل...')); ?></div>
                    </div>
                </div>
                <div class="p-2 border-top text-center">
                    <a href="#" class="small text-decoration-none fw-bold text-muted"><?php echo e(__('عرض كل الإشعارات')); ?></a>
                </div>
            </div>
        </div>
...
<?php /**PATH D:\Projects\XO\Zad\resources\views/partials/crm-topbar.blade.php ENDPATH**/ ?>