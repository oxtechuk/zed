                ...
                <div class="card border-0 shadow-sm mb-4 rounded-4 overflow-hidden">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="content[ar]" class="form-control bg-light border-0 shadow-none" rows="10">{{ old('content.ar', $blog->getTranslation('content', 'ar')) }}</textarea>
                        </div>
                        <div class="mb-0">
                            <label class="form-label fw-bold small text-muted">{{ __('المحتوى الكامل (EN)') }} <span class="text-danger">*</span></label>
                            <textarea name="content[en]" class="form-control bg-light border-0 shadow-none" rows="10">{{ old('content.en', $blog->getTranslation('content', 'en')) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                    ...
                    <div class="card-body p-4 pt-2">
                        <select name="tag" class="form-select bg-light border-0 shadow-none">
                            ...
                            <option value="new" {{ old('tag', $blog->tag) == 'new' ? 'selected' : '' }}>{{ __('جديد') }}</option>
                            <option value="popular" {{ old('tag', $blog->tag) == 'popular' ? 'selected' : '' }}>{{ __('شائع') }}</option>
                            <option value="exclusive" {{ old('tag', $blog->tag) == 'exclusive' ? 'selected' : '' }}>{{ __('حصري') }}</option>
                            <option value="limited" {{ old('tag', $blog->tag) == 'limited' ? 'selected' : '' }}>{{ __('محدود') }}</option>
                        </select>
                    </div>
                </div>
