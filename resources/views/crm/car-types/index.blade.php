...
@section('title', __('أنواع السيارات') . ' | AutoCRM')
...
@section('content')
    <div class="container-fluid" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <h4 class="mb-1 fw-bold"> {{ __('أنواع السيارات') }}</h4>
                <p class="text-muted mb-0 small">{{ __('إجمالي') }} {{ $types->total() }} {{ __('نوع متاح في النظام') }}</p>
            </div>
        ...
        <div class="row g-4">
            @forelse ($types as $type)
                <div class="col-6 col-md-4 col-xl-3">
                    <div class="card border-0 shadow-sm text-center h-100 rounded-4 overflow-hidden transition-hover">
                        <div class="card-body p-4">
                            ...
                            <p class="text-muted small mb-3 fw-medium">{{ $type->cars_count ?? 0 }} {{ __('سيارة') }}</p>
                            @if (!$type->is_active)
                        ...
                        <div class="card-footer bg-light border-0 d-flex gap-2 p-3">
                            ...
                            <button class="btn btn-sm btn-white border shadow-xs flex-grow-1 rounded-3 fw-bold" data-bs-toggle="modal"
                                data-bs-target="#editType{{ $type->id }}"><i class="bi bi-pencil-square me-1"></i> {{ __('تعديل') }}</button>
                            @endcan
                            @can('manage-car-types')
                            <form action="{{ route('crm.car-types.destroy', $type) }}" method="POST"
                ...
                <div class="modal fade" id="editType{{ $type->id }}" tabindex="-1">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                            <div class="modal-header border-0 pt-4 px-4">
                                <h5 class="modal-title fw-bold">{{ __('تعديل النوع') }}</h5>
                                <button type="button" class="btn-close {{ app()->getLocale() == 'ar' ? 'ms-0 me-auto' : '' }}" data-bs-dismiss="modal"></button>
                            </div>
                            <form action="{{ route('crm.car-types.update', $type) }}" method="POST">
                                @csrf @method('PUT')
                                <div class="modal-body p-4 pt-2">
                                    <ul class="nav nav-pills nav-fill bg-light p-1 rounded-pill mb-4" role="tablist">
                                        <li class="nav-item">
                                            <button class="nav-link active rounded-pill py-1 fw-bold" data-bs-toggle="tab" data-bs-target="#edit-ar-type-{{ $type->id }}" type="button">{{ __('العربية') }}</button>
                                        </li>
                                        <li class="nav-item">
                                            <button class="nav-link rounded-pill py-1 fw-bold" data-bs-toggle="tab" data-bs-target="#edit-en-type-{{ $type->id }}" type="button">{{ __('الإنجليزية') }}</button>
                                        </li>
                                    </ul>

                                    <div class="tab-content mb-4">
                                        <div class="tab-pane fade show active" id="edit-ar-type-{{ $type->id }}">
                                        ...
                                        <div class="tab-pane fade" id="edit-en-type-{{ $type->id }}">
                                    ...
                                    <div class="form-check form-switch p-3 bg-light rounded-3 border-0">
                                        <input class="form-check-input {{ app()->getLocale() == 'ar' ? 'ms-0 me-2 float-none' : '' }}" type="checkbox" name="is_active" value="1"
                                            id="tp{{ $type->id }}" {{ $type->is_active ? 'checked' : '' }}>
                                        <label class="form-check-label fw-bold ms-2" for="tp{{ $type->id }}">{{ __('تفعيل النوع في الموقع') }}</label>
                                    </div>
                                </div>
            ...
            @empty
                <div class="col-12 text-center py-5">
                    <div class="text-center py-5 bg-white rounded-4 shadow-sm opacity-50">
                        ...
                        <h6 class="fw-bold">{{ __('لا توجد أنواع مسجلة حالياً') }}</h6>
                        <p class="small">{{ __('قم بإضافة أنواع مثل (سيدان، SUV، هاتشباك) لتصنيف سياراتك') }}</p>
                    </div>
                </div>
            @endforelse
        </div>

        <div class="mt-5 d-flex justify-content-center">{{ $types->links() }}</div>

        <div class="modal fade" id="addTypeModal" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                    <div class="modal-header border-0 pt-4 px-4">
                        <h5 class="modal-title fw-bold">{{ __('إضافة نوع جديد') }}</h5>
                        <button type="button" class="btn-close {{ app()->getLocale() == 'ar' ? 'ms-0 me-auto' : '' }}" data-bs-dismiss="modal"></button>
                    </div>
                    <form action="{{ route('crm.car-types.store') }}" method="POST">
                        @csrf
                        <div class="modal-body p-4 pt-2">
                            <ul class="nav nav-pills nav-fill bg-light p-1 rounded-pill mb-4" role="tablist">
                                <li class="nav-item">
                                    <button class="nav-link active rounded-pill py-1 fw-bold" data-bs-toggle="tab" data-bs-target="#add-ar-type" type="button">{{ __('العربية') }}</button>
                                </li>
                                <li class="nav-item">
                                    <button class="nav-link rounded-pill py-1 fw-bold" data-bs-toggle="tab" data-bs-target="#add-en-type" type="button">{{ __('الإنجليزية') }}</button>
                                </li>
                            </ul>
                            ...
                            <div class="form-check form-switch p-3 bg-light rounded-3 border-0">
                                <input class="form-check-input {{ app()->getLocale() == 'ar' ? 'ms-0 me-2 float-none' : '' }}" type="checkbox" name="is_active" value="1" id="nctp" checked>
                                <label class="form-check-label fw-bold ms-2" for="nctp">{{ __('تفعيل النوع فوراً') }}</label>
                            </div>
                        </div>
                        <div class="modal-footer border-0 p-4 pt-0">
                            <button type="button" class="btn btn-light rounded-pill px-4" data-bs-dismiss="modal">{{ __('إلغاء') }}</button>
