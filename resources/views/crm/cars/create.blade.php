    ...
    <form action="{{ route('crm.cars.store') }}" method="POST" enctype="multipart/form-data" id="car-form">
        ...
        <div class="row g-4">
            ...
            <div class="col-12 col-lg-8">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <button type="button" class="btn-quick-add-link" data-bs-toggle="modal" data-bs-target="#quickAddModelModal" title="{{ __('إضافة موديل جديد') }}" id="btn-quick-add-model" disabled>
                                        <i class="bi bi-plus-circle-fill"></i> {{ __('إضافة سريعة') }}
                                    </button>
                                </div>
                                <select name="car_model_id" id="car_model_id" class="form-select @error('car_model_id') is-invalid @enderror" required>
                                    <option value="">{{ __('اختر الماركة أولاً') }}</option>
                                </select>
                ...
                <div class="car-section mb-4">
                    <div class="car-section-header">
                        <i class="bi bi-translate"></i> {{ __('اسم السيارة والوصف') }}
                    </div>
                    <div class="car-section-body">
                        <div class="row g-3">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                <textarea name="features[ar]" class="form-control" rows="3" dir="rtl" placeholder="{{ __('أبرز مميزات السيارة كنص...') }}">{{ old('features.ar') }}</textarea>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">{{ __('المميزات العامة (نص إنجليزي)') }}</label>
                                <textarea name="features[en]" class="form-control" rows="3" dir="ltr" placeholder="{{ __('Key features in English...') }}">{{ old('features.en') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <form id="quick-add-model-form" action="{{ route('crm.car-models.store') }}" method="POST">
                ...
                <div class="modal-body p-4 pt-2">
                    <div class="mb-3">
                        ...
                        <input type="text" id="quick_add_brand_name" class="form-control bg-light border-0 shadow-none fw-bold text-primary" readonly>
                    </div>
        ...
        success: function(data) {
            ...
            if (Array.isArray(data) && data.length > 0) {
                $.each(data, function(index, model) {
                    const isSelected = (targetSelectedId && model.id == targetSelectedId) || (model.id == "{{ old('car_model_id') }}");
...
// If brand already selected on page load (e.g. validation redirect)
if ($('#brand_id').val()) {
    fetchModelsForBrand($('#brand_id').val(), "{{ old('car_model_id') }}");
}
