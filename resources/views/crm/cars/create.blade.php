...
@section('title', __('إضافة سيارة جديدة') . ' | Zad Capital')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <button type="button" class="btn btn-outline-primary btn-sm rounded-3 px-3" data-bs-toggle="modal" data-bs-target="#quickAddModelModal" title="{{ __('إضافة موديل جديد') }}" id="btn-quick-add-model" disabled>
...
$('#brand_id').on('change', function() {
    ...
    modelSelect.html('<option value="">{{ __("جاري التحميل...") }}</option>');
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data, function(index, model) {
                modelSelect.append(`<option value="${model.id}" ${model.id == "{{ old('car_model_id') }}" ? 'selected' : ''}>${model.name}</option>`);
            });
        },
        error: function() {
            modelSelect.html('<option value="">{{ __("خطأ في تحميل الموديلات") }}</option>');
        }
    });
});
...
$('#quick-add-model-form').on('submit', function(e) {
    ...
    btn.prop('disabled', true).text('{{ __("جاري الحفظ...") }}');

    $.ajax({
        url: '{{ route("crm.car-models.store") }}',
        type: 'POST',
        data: form.serialize(),
        success: function(response) {
            if (response.success) {
                ...
                alert(response.message || '{{ __("حدث خطأ ما") }}');
            }
        },
        error: function(xhr) {
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden text-start">
            <div class="modal-header border-0 pt-4 px-4">
                <h5 class="modal-title fw-bold" id="quickAddModelModalLabel">{{ __('إضافة موديل جديد') }}</h5>
            ...
            <form id="quick-add-model-form">
                ...
                <div class="modal-body p-4 pt-2">
                    ...
                    <ul class="nav nav-pills nav-fill bg-light p-1 rounded-pill mb-4" role="tablist">
                        <li class="nav-item">
                            <button class="nav-link active rounded-pill py-1 fw-bold" data-bs-toggle="tab" data-bs-target="#quick-add-ar" type="button" role="tab">{{ __('العربية') }}</button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link rounded-pill py-1 fw-bold" data-bs-toggle="tab" data-bs-target="#quick-add-en" type="button" role="tab">{{ __('الإنجليزية') }}</button>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content mb-4">
                        <div class="tab-pane fade show active" id="quick-add-ar" role="tabpanel">
                            ...
                            <input type="text" name="name[ar]" id="quick_add_name_ar" class="form-control bg-light border-0 shadow-none" placeholder="{{ __('مثال: كامري') }}" required>
                        </div>
                        <div class="tab-pane fade" id="quick-add-en" role="tabpanel">
                            ...
                            <input type="text" name="name[en]" id="quick_add_name_en" class="form-control bg-light border-0 shadow-none" placeholder="{{ __('e.g., Camry') }}" required>
                        </div>
                    </div>
                </div>
