...
export default function AboutPageSkeleton({ className = "" }: IAboutPageSkeletonProps) {
  return (
    <main
      aria-busy="true"
      aria-label="Loading about page"
      className={`min-h-screen select-none bg-[#080E1E] text-white ${className}`}
    >
      {/* ── Hero Section ── */}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-9 grid grid-cols-2 border-t border-white/10 md:grid-cols-4">
            {Array.from({ length: 4 }).map((_, idx) => (
              <div key={idx} className="flex min-h-[92px] flex-col items-center justify-center p-5">
      ...
      <section className="w-full bg-[#080E1E] py-12 sm:py-16 lg:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 flex gap-5 overflow-hidden">
            {Array.from({ length: 4 }).map((_, idx) => (
              <Skeleton key={idx} className="h-[360px] w-full sm:w-[calc((100%-3*1.25rem)/4)] rounded-[16px] bg-white/10 shrink-0" />
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-around gap-6">
            {Array.from({ length: 5 }).map((_, idx) => (
              <Skeleton key={idx} className="h-16 w-32 rounded-xl bg-gray-200" />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="w-full bg-[#010915] py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            {Array.from({ length: 3 }).map((_, idx) => (
              <Skeleton key={idx} className="h-56 w-full rounded-2xl bg-white/10" />
            ))}
          </div>
        </div>
      </section>
