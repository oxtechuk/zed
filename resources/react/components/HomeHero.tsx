...
export default function HomeHero({ slides = [] }: IHomeHeroProps) {
  ...
  return (
    <section className="w-full pb-8 pt-0" dir={direction}>
      ...
      <div className="relative w-full overflow-hidden bg-[#051023] min-h-[220px] max-h-[360px] h-[58vw] sm:min-h-[380px] sm:max-h-none sm:h-[440px] md:h-[490px] lg:h-[540px]">
                ...
                {hasTextContent && (
                  <div className="relative z-10 mx-auto max-w-7xl px-5 sm:px-10 lg:px-12 w-full h-full flex flex-col justify-center items-start text-start py-4 sm:py-0">
                    <div className="max-w-xl">
                      {slide.badge && (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 mb-2 sm:mb-3.5 rounded-full text-[10px] sm:text-[12px] font-black bg-[#EDC98E] text-[#16254F] shadow-sm tracking-wide">
                          {slide.badge}
                        </span>
                      )}

                      {slide.title && (
                        <h1 className="text-[18px] sm:text-[32px] md:text-[40px] lg:text-[46px] font-black text-white leading-[1.2] sm:leading-tight drop-shadow-md">
                          {slide.title}
                        </h1>
                      )}

                      {(slide.subtitle || slide.description) && (
                        <p className="mt-1.5 sm:mt-3 text-[12px] sm:text-[15px] md:text-[16px] text-gray-200/90 font-medium leading-relaxed max-w-lg line-clamp-2 sm:line-clamp-3 drop-shadow-sm">
                          {slide.subtitle || slide.description}
                        </p>
                      )}

                      {/* Two CTA Action Buttons */}
                      <div className="mt-3.5 sm:mt-7 flex flex-wrap items-center gap-2 sm:gap-4">
                        {/* Button 1: احسب تمويلك */}
                        <Link
                          to="/finance-calculator"
                          className="h-[36px] sm:h-[48px] px-4 sm:px-7 rounded-xl sm:rounded-2xl bg-[#EDC98E] text-[#16254F] hover:bg-[#e2bc7c] font-black text-[12px] sm:text-[15px] flex items-center justify-center gap-1.5 sm:gap-2 shadow-[0_8px_20px_rgba(237,201,142,0.25)] hover:scale-105 transition-all duration-300 active:scale-95 text-decoration-none"
                        >
                        ...
                        <Link
                          to="/cars"
                          className="h-[36px] sm:h-[48px] px-3.5 sm:px-6 rounded-xl sm:rounded-2xl bg-[#0B1528]/80 hover:bg-[#16254F] border border-white/20 text-white font-bold text-[12px] sm:text-[15px] flex items-center justify-center gap-1.5 sm:gap-2 hover:scale-105 transition-all duration-300 active:scale-95 text-decoration-none backdrop-blur-md"
                        >
                          <Car size={15} className="text-[#EDC98E]" />
                          <span>
                            {t("hero.browseCars", {
