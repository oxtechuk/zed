...
interface IStepTwoCarSelectorProps {
...
export default function StepTwoCarSelector({
  ...
  const handleNext = () => {
    if (!selectedCarId) {
      toast.error(t("financeCalculator.validation.selectCar", { defaultValue: "الرجاء اختيار سيارة للمتابعة" }));
      return;
    }
    onNext();
  };
    ...
    <div dir={i18n.dir()} className="w-full max-w-3xl mx-auto">
      <div className="rounded-[24px] border border-[#E5E9F0] bg-white px-6 py-8 shadow-sm md:px-10">
        ...
        <div className="text-center mb-6">
          <h2 className="text-[22px] font-black text-[#0F172A]">اختر السيارة</h2>
          <p className="text-[13px] text-gray-400 font-bold mt-1">حدد السيارة التي تريد تمويلها</p>
        </div>
        ...
        <div className="relative mb-6">
          {showSearch ? (
            <div className="relative z-25">
              <input
                type="text"
                placeholder="اكتب اسم السيارة، الماركة، أو الموديل..."
                value={searchQuery}
              ...
              <button
                ...
                className="absolute end-4 top-1/2 -translate-y-1/2 text-[#8A8F99] hover:text-[#5F6672]"
              >
                <X size={18} />
              </button>
              ...
              {searchResults.length > 0 && (
                <div className="absolute left-0 right-0 z-30 mt-2 max-h-[250px] overflow-y-auto rounded-2xl border border-[#E5E9F0] bg-white shadow-xl">
                  {searchResults.map((car) => (
                    <button
                      ...
                      className="flex w-full items-center gap-4 px-5 py-4 text-right transition hover:bg-[#F8FAFC] border-b border-[#EEF2F6] last:border-0"
                    >
                      <img
              ...
              {!searching && searchResults.length === 0 && (
                <div className="absolute left-0 right-0 z-30 mt-2 p-5 rounded-2xl border border-[#E5E9F0] bg-white text-center text-[13px] text-gray-400 font-bold shadow-lg">
            ...
            <button
              type="button"
              onClick={() => setShowSearch(true)}
              className="flex h-[52px] w-full items-center justify-between rounded-2xl border border-[#E5E9F0] bg-white px-5 text-[14px] font-bold text-gray-400 transition hover:border-gray-400"
            >
              <span>اختر السيارة التي تريد تمويلها...</span>
              <ChevronDown size={18} className="text-[#8A8F99]" />
            </button>
          )}
        </div>
        ...
        {/* Car Image Preview */}
        {selectedCarId > 0 && (
          <div className="mb-6 rounded-[20px] bg-white border border-[#E5E9F0] p-6 shadow-xs flex items-center justify-center min-h-[220px] md:min-h-[300px]">
            <img
        ...
        {selectedCarId > 0 && (
          <div className="mb-8 rounded-2xl bg-[#0F172A] p-5 text-white flex items-center justify-between text-start relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#EDC98E]/5 blur-xl rounded-full" />
            <div>
              <span className="text-[12px] text-white/50 font-bold block mb-1">
                سعر السيارة
              </span>
              <strong className="text-[24px] font-black leading-none text-white tracking-tight">
                {formatPrice(selectedCar.price, "white")}
              </strong>
            </div>
          </div>
        )}
        ...
        <button
          type="button"
          onClick={handleNext}
          className="flex h-[52px] w-full items-center justify-center gap-2 rounded-xl bg-[#0F172A] text-[15px] font-extrabold text-white transition hover:opacity-95 hover:scale-[1.01] active:scale-95 shadow-sm"
        >
          <span>التالي</span>
